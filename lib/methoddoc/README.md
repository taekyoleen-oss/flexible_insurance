...
| `N. 별첨 — 위험률 표` (`연령 \| 위험률(남) \| …` 값 표) | 조건으로 읽지 않는다(`isRateValueTable` — 열 이름이 위험률로 주워지지 않게 뺀다). 앱이 위험률 표 창으로 가져간다(`sheetFromDoc`). 값 표가 붙은 위험률이 있을 때만 render 가 맨 뒤에 싣는다(`rateGrid`) |
| 맨 앞 `작성 안내` 표 | 읽지 않는다 |

담보 표에 적은 위험률 이름이 1.2. 위험률 표에 없으면 계열로 더하고(`유형` 은 이름으로 어림) `warnings` 로 알린다.
문서를 고쳐 다시 넣는 쪽(`mergeSpec`, 앱)은 표준 양식이면 담보 표에서 지운 칸(면책·구간)과 위험률 표에서 지운 행(담보가 쓰지 않는 것)도 뺀다.
