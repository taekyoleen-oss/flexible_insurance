 ...
 * l_{t+1}  = l_t·(1 − q − w + q·w/2)          급부 집단(사망·해지 탈퇴)
 * l′_{t+1} = l′_t·(1 − q − f − w + (qf+qw+fw)/2)  납입 집단(+ 납입면제 탈퇴)
 * C_t = l_t·q·(1 − w/2)·v^{t+½}, W_t = l_t·w·v^{t+½}  — 사망·해지 모두 연중앙
...
export function commutation(basis: Basis, age: number, n: number): Commutation {
  ...
  for (let t = 0; t < n; t++) {
    ...
    lx[t + 1] = lx[t] * (1 - q - w + (q * w) / 2);
    lxp[t + 1] = lxp[t] * (1 - q - f - w + (q * f + q * w + f * w) / 2);
  }
  const Dx = new Array<number>(len), Dpx = new Array<number>(len), Cx = new Array<number>(len), Wx = new Array<number>(len);
  for (let t = 0; t < len; t++) {
    const q = basis.q[age + t] ?? 0, w = wAt(t);
    Dx[t] = lx[t] * v ** t;
    Dpx[t] = lxp[t] * v ** t;
    Cx[t] = lx[t] * q * (1 - w / 2) * v ** (t + 0.5);
    Wx[t] = lx[t] * w * v ** (t + 0.5);
  }
