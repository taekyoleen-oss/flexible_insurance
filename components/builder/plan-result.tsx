...
export function PremiumCard() {
  const { state: s, product: r } = usePlan();
  const eff = r.effective, low = r.low;
  const freq = s.base.freq;
  ...
  return (
    <Card title="보험료">
      <div className="mb-3">
        <div className="text-xs text-navy/60">{freq === 12 ? "월" : freq === 1 ? "연" : `${12 / freq}개월`} 영업보험료 (전 탭 합계)</div>
        <div className="font-mono text-3xl text-navy">{won(eff.monthlyGross)}</div>
      </div>
      <table className="w-full text-sm">
        <thead className="text-xs text-navy/60">
          <tr><th className="pb-1 text-left font-medium">탭 · 담보</th><th className="pb-1 text-right font-medium">보장금액</th><th className="pb-1 text-right font-medium">순보험료</th><th className="pb-1 text-right font-medium">영업보험료</th></tr>
        </thead>
        <tbody>
          {r.coverages.map((c, i) => (
            <tr key={c.id} className="border-t border-navy/10">
              <td className="py-1"><span className="inline-block h-2 w-2 rounded-full align-middle" style={{ background: SERIES[i % SERIES.length] }} /> <span className="text-xs text-navy/45">{c.tabName}</span> {c.label}<span className="ml-1 text-xs text-navy/50">{kindMeta(c.kind).label} · {c.n}년</span></td>
      ...
      <dl className="mt-3 grid grid-cols-[1fr_auto] gap-y-1 border-t border-navy/10 pt-2 text-sm">
        ...
        {low && <><dt className="text-navy/60">저해지·무해지 적용 후</dt><dd className="font-mono">−{pct(low.premiumDiscount)}</dd></>}
      </dl>
    </Card>
  );
}
      ...
      <div className="h-64">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 8, right: 16, bottom: 8, left: 8 }}>
            ...
            <Legend />
            {r.coverages.map((c, i) => (
              <Area key={c.id} dataKey={c.id} name={r.tabs.length > 1 ? `${c.tabName} · ${c.label}` : c.label} stackId="1" type="stepAfter"
  ...
  return (
    <details className="rounded-lg border border-navy/10 bg-white p-4 shadow-sm">
      ...
      <p className="mt-2 text-xs text-navy/60">
        ...
        주계약 기준 예정이율 {pct(s.base.interest, 2)} · 표준이율 {pct(s.base.standardInterest, 2)} · 납입면제 {s.base.waiver ? "적용" : "미적용"} (특약이 다르게 둔 경우 그 탭 값으로 산출합니다).
      </p>
      <div className="mt-3 overflow-auto">
        <table className="w-full text-xs">
          <thead className="text-navy/60">
            <tr>
              {["탭 · 담보", "보장기간", "급부현가 PVB", "납입기수 N*", "순 P(10만원당)", "영업 G(10만원당)", "신계약비 α", "해약공제 기준", r.low ? "해지급부현가 CSV₀" : ""].filter(Boolean).map((h) => <th key={h} className="py-1 text-right font-medium first:text-left">{h}</th>)}
            </tr>
          </thead>
          <tbody>
            {r.coverages.map((c) => (
              <Fragment key={c.id}>
                <tr className="border-t border-navy/10 font-mono">
                  <td className="py-1 font-sans"><span className="text-navy/45">{c.tabName}</span> {c.label}</td>
